 ...
 *
 * Modifyed by Pascal Gollor (https://www.pgollor.de/cms/):
 * - time request answer for local time
 *
...
void receive(const MyMessage &message)
{
	...
	{
		std::time_t tLocal = time(0) + localTimeDiff;
		MyMessage timeMessage;

		// prepare message
		timeMessage.set(static_cast<uint32_t>(tLocal));
		...
		DEBUG_OUTPUT("Reqeust time from node: %i - local seconds since 1970: %d\n", message.sender, static_cast<uint32_t>(tLocal));
		...
		gatewayTransportSend(timeMessage);
#endif
	}
}
...
void setup()
{
	...
	// calculate local time diff
	std::time_t tCurrent = std::time(0);
	std::time_t tUTC = mktime(gmtime(&tCurrent));
	std::tm *tmLocal = localtime(&tCurrent);
	std::time_t tLocal = mktime(tmLocal);

	localTimeDiff = difftime(tLocal, tUTC);

	// add one hour for summer time (daylight saving time)
	if (tmLocal->tm_isdst)
	{
		localTimeDiff += 3600;
	}
